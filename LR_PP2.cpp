...
long double input_check()
{
	...
	*/
	...
	while (ok)
	{
		...
		{
			cerr << "Error(2)! Write a positive number!" << endl;
		}
...
void output(int n, vector <long double> An, vector <long double> Sn, vector <long double> En)          //это вывод
{
	if (abs(En[n]) > numeric_limits<long double>::min())
	cout << "[ n = " << setw(10) << n << " ][ " << "An = " << setw(10) << An[n] << " ][ " << "Sn = " << setw(10) << Sn[n] << " ][ "
		<< "En = " << setw(10) << En[n] << " ]" << '\n';
}
	...
	while (finish)  
	{
		...
		if (x0 == x)    //если предыдущее значение совпадает с текущим
		{
			...
			else  //a - погрешность
			{
				...
				do
				{
					if (n < An.size())   output(n, An, Sn, En);
					else
					{
						vector <long double> data = iteration_calc(n, x, Sn[n - 1]);
						An.push_back(data[0]);	Sn.push_back(data[1]);   En.push_back(data[2]); length = n;
